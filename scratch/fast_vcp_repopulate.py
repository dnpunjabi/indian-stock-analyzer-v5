...
async def run_fast_vcp_repopulation():
    print("Fetching universe for fast parallel VCP scan...")
    with get_db() as conn:
        cursor = conn.cursor()
    ...
    # Clear old vcp_screener_cache table
    with get_db() as conn:
    ...
    async def scan_single(st):
        async with sem:
            sym = st["symbol"].strip().upper()
            yf_sym = f"{sym}.NS" if not sym.endswith(".NS") else sym
            
            loop = asyncio.get_running_loop()
            df = await loop.run_in_executor(None, fetch_fast_df, yf_sym)
            ...
            def get_canslim():
                with get_db() as conn:
                    return calculate_canslim_score(sym, conn, df=df)

            canslim_res = await loop.run_in_executor(None, get_canslim)
    ...
    tasks = [scan_single(st) for st in stocks]
    results = await asyncio.gather(*tasks)
    clean_vcp_results = [r for r in results if r is not None]
...
if __name__ == "__main__":
    asyncio.run(run_fast_vcp_repopulation())
