...
def calculate_mtf_matrix_alignment(df) -> dict:
    """
    Evaluates Multi-Timeframe Matrix Alignment across Daily, Weekly, and Monthly timeframes.
    
    1. Monthly Matrix: Close > 10 MMA > 20 MMA (Secular Bull Anchor)
    2. Weekly Matrix: Close > 10 WMA > 30 WMA & 30 WMA Slope > 0 (Stage 2 Mark-Up)
    3. Daily Matrix: Close > 10 EMA > 21 EMA > 50 SMA (Bullish Stack Order)
    4. Volatility Squeeze: 5-Day ATR % < 2.5% (Tight Compression)
    """
    default_res = {
        "is_mtf_aligned": False,
        "score": 0,
        "status": "NONE",
        "monthly_regime": "NEUTRAL",
        "weekly_stage2": False,
        "daily_stack": False,
        "atr_pct": 0.0,
        "weekly_slope": 0.0,
    ...
    try:
        ...
        d_close = float(df_work['Close'].iloc[-1])
        d_10ema = float(df_work['Close'].ewm(span=10, adjust=False).mean().iloc[-1])
        d_21ema = float(df_work['Close'].ewm(span=21, adjust=False).mean().iloc[-1])
        d_50sma = float(df_work['Close'].rolling(50, min_periods=20).mean().iloc[-1])
        ...
        if 'High' in df_work.columns and 'Low' in df_work.columns:
            ...
            atr_pct = float((atr5 / d_close) * 100.0) if d_close > 0 else 0.0
        else:
            atr_pct = 2.0

        daily_stack_pass = (d_close >= d_10ema >= d_21ema >= d_50sma)
        daily_squeeze_pass = (atr_pct <= 3.5)

        # 2. Weekly Timeframe Resampling
        ...
        if len(df_weekly) >= 15:
            ...
            w_10wma = float(df_weekly['Close'].rolling(10, min_periods=5).mean().iloc[-1])
            w_30wma = float(df_weekly['Close'].rolling(30, min_periods=10).mean().iloc[-1])
            
            w_30wma_prev = float(df_weekly['Close'].rolling(30, min_periods=10).mean().iloc[-min(5, len(df_weekly))])
            if w_30wma_prev > 0:
                w_slope = ((w_30wma - w_30wma_prev) / w_30wma_prev) * 100.0
            
            weekly_pass = (w_close >= w_10wma >= w_30wma) and (w_slope >= -0.1)
        elif len(df_weekly) >= 3:
            w_close = float(df_weekly['Close'].iloc[-1])
            w_prev = float(df_weekly['Close'].iloc[-2])
            weekly_pass = (w_close >= w_prev)

        # 3. Monthly Timeframe Resampling
        df_monthly = df_work.resample('ME').agg({
            'Open': 'first',
            'High': 'max',
            'Low': 'min',
            'Close': 'last',
        ...
        monthly_pass = False
        if len(df_monthly) >= 5:
            m_close = float(df_monthly['Close'].iloc[-1])
            m_10mma = float(df_monthly['Close'].ewm(span=10, adjust=False).mean().iloc[-1])
            m_20mma = float(df_monthly['Close'].ewm(span=20, adjust=False).mean().iloc[-1])
            monthly_pass = (m_close >= m_10mma >= m_20mma)
        elif len(df_monthly) >= 2:
            m_close = float(df_monthly['Close'].iloc[-1])
            m_prev = float(df_monthly['Close'].iloc[-2])
            monthly_pass = (m_close >= m_prev)

        # 4. Confluence Scoring
        score = 0
        if daily_stack_pass: score += 25
        if daily_squeeze_pass: score += 15
        if weekly_pass: score += 35
        if monthly_pass: score += 25

        is_aligned = (daily_stack_pass and weekly_pass and monthly_pass)

        if is_aligned and atr_pct <= 2.5:
            status = "APEX_ALIGNED"
            reason = "100% Tri-MA Matrix Aligned with Tight Compression"
        elif is_aligned:
            status = "TRI_MA_ALIGNED"
            reason = "100% Tri-MA Matrix Aligned"
        elif score >= 60:
            status = "PARTIAL_MATRIX"
            reason = "Partial Timeframe Alignment"
        else:
            status = "UNALIGNED"
            reason = "Failed Timeframe Alignment Matrix"

        return {
            "is_mtf_aligned": is_aligned,
            "score": score,
            "status": status,
            "monthly_regime": "BULLISH" if monthly_pass else "NEUTRAL",
            "weekly_stage2": weekly_pass,
            "daily_stack": daily_stack_pass,
            "atr_pct": round(atr_pct, 2),
            "weekly_slope": round(w_slope, 2),
            "reason": reason
        }
    except Exception as e:
        print(f"Error calculating mtf_matrix_alignment: {e}")
        return default_res
...
def detect_flat_base_breakout(df: pd.DataFrame, rs_score: float = 0.0) -> dict:
    """
    Detects William O'Neil / Dan Zanger / Nicolas Darvas Modern Flat Base Breakout:
    1. Base Duration: Dynamic 4 to 15 weeks (20 to 75 trading days) from actual base ceiling peak. ZERO FALLBACKS.
    2. Base Depth: Max pullback depth strictly <= 15.0% (4.0% to 15.0%).
    3. Proximity: Base ceiling within 20.0% of 52-week High.
    4. Prior Trend: Prior advance >= +25.0% into the base peak.
    5. Trend Template: Price >= 50-day SMA >= 200-day SMA with 200-day SMA rising over 30 days.
    6. Volume Dry-Up (VDU): Base volume <= 0.85x 50d avg volume.
    7. Relative Strength Leadership: RS Rating >= 70.0 or positive Mansfield RS.
    8. Breakout Status: LIVE_BREAKOUT (Close >= Pivot and Vol >= 1.3x), READY_PIVOT (within 2.5% & VDU <= 0.85x), FORMING.
    """
    default_res = {
        "is_flat_base": False,
        "base_status": "NONE",
    ...
    try:
        ...
        # 1. 52-Week High Proximity (Ceiling / price within 20.0% of 52W High)
        ...
        print(f"Error in detect_flat_base_breakout: {e}")
